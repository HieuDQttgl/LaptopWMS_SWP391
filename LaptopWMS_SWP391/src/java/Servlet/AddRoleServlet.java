    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       String roleDescription = request.getParameter("roleDescription");
        if (roleName == null || roleName.trim().isEmpty()) {
            request.setAttribute("error", "Role name cant be empty");
            request.getRequestDispatcher("add-role.jsp").forward(request, response);
            return;
        }
        ...
        try {
            boolean success = dao.addRole(roleName.trim(), roleDescription);
            if (success) {
                response.sendRedirect("role-permission?status=addSuccess"); 
            } else {
                request.setAttribute("error", "Role **'" + roleName + "'** already existed");
                request.getRequestDispatcher("add-role.jsp").forward(request, response);
            }
            
        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("error", "Cant connect to database");
            request.getRequestDispatcher("add-role.jsp").forward(request, response);
        }
